...
const NewsCard = ({ news }) => {
  ...
  return (
    <motion.div
      ...
      className="dept-content-card group relative flex flex-col overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm transition-all duration-300"
    >
      {news.image_path && (
        <div className="relative aspect-[16/9] overflow-hidden bg-gray-100">
          ...
          <div className="dept-date-pill absolute right-4 top-4 rounded-full bg-white/95 px-3 py-1.5 text-xs font-semibold shadow-sm backdrop-blur-sm">
      ...
      <div className="flex flex-grow flex-col p-6">
        <h3 className="dept-title-hover mb-3 line-clamp-2 text-xl font-bold text-gray-900 transition-colors">
          {news.title}
        </h3>
        ...
        >
          <motion.span
            className="dept-link group/link inline-flex items-center text-sm font-medium"
            whileHover={{ x: 5 }}
      ...
      <div className="dept-card-bottom-line pointer-events-none absolute bottom-0 left-0 h-1 w-full opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
    </motion.div>
  );
};
