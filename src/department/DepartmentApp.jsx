...
function DepartmentPublicLayout() {
  const { deptKey, deptInfo } = useDepartment();

  return (
    <div
      className={`dept-site dept-site-${deptKey}`}
      style={{
        "--dept-primary": deptInfo?.color || "#a6192e",
        "--dept-primary-soft": `rgba(${deptInfo?.colorRgb || "166,25,46"},0.12)`,
        "--dept-primary-border": `rgba(${deptInfo?.colorRgb || "166,25,46"},0.28)`,
      }}
    >
      <Navbar />
      <Outlet />
